...
(defun flycheck-error-format-position (err)
  ...
  (let ((line (flycheck-error-line err))
        ...
        (end-column (flycheck-error-end-column err)))
    (cond
     ((and line column end-column)
      (if (or (equal line end-line) (null end-line))
          (if (equal column end-column)
              (format "%d:%d" line column)
            (format "%d:%d-%d" line column end-column))
        (format "(%d:%d)-(%d:%d)" line column end-line end-column)))
     ((and line column) (format "%d:%d" line column))
     (t (format "%d" line)))))
      ...
      (let ((filename (match-string 1 err))
        ...
        (flycheck-error-new-at
         ...
         :filename (if (or (null filename) (string-empty-p filename))
                       (buffer-file-name)
                     filename)
         ;; `end-line' defaults to line if there's a column, since checkers
         ;; often omit the end-line (as in <file>:<line>:<column>-<end-column>)
...
See URL `https://jsonnet.org'."
  ...
  :error-patterns
  ;; FIXME: jsonnet can also report errors as (line-col):(line-col)
  ((error line-start "STATIC ERROR: " (file-name) ":" line ":" column
          (zero-or-one (seq "-" end-column)) ": "
          (message) line-end)
   (error line-start "RUNTIME ERROR: " (message) "\n"
          (one-or-more space) (file-name) ":" line ":" column
          (zero-or-one (seq "-" end-column))
          (zero-or-more not-newline) line-end))
  :modes jsonnet-mode)
